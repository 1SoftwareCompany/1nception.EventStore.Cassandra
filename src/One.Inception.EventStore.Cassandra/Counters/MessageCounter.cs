...
namespace One.Inception.EventStore.Cassandra.Counters
{
    internal class CounterIncrementQuery : PreparedStatementCache
    {
        ...
        public CounterIncrementQuery(IInceptionContextAccessor context, ICassandraProvider cassandraProvider) : base(context, cassandraProvider) { }
    ...
    internal class CounterDecrementQuery : PreparedStatementCache
    {
        ...
        public CounterDecrementQuery(IInceptionContextAccessor context, ICassandraProvider cassandraProvider) : base(context, cassandraProvider) { }
    ...
    internal class CounterLoadQuery : PreparedStatementCache
    {
        ...
        public CounterLoadQuery(IInceptionContextAccessor context, ICassandraProvider cassandraProvider) : base(context, cassandraProvider) { }
    ...
    public class MessageCounter : IMessageCounter
    {
        ...
        public MessageCounter(IInceptionContextAccessor contextAccessor, ICassandraProvider cassandraProvider, ILogger<MessageCounter> logger)
        {
            this.cassandraProvider = cassandraProvider;
            this.logger = logger;

            _incrementTemplatePreparedStatement = new CounterIncrementQuery(contextAccessor, cassandraProvider);
            _decrementTemplatePreparedStatement = new CounterDecrementQuery(contextAccessor, cassandraProvider);
            _getTempletePreparedStatement = new CounterLoadQuery(contextAccessor, cassandraProvider);
        }
        ...
        public async Task IncrementAsync(Type messageType, long incrementWith = 1)
        {
            ...
            catch (Exception ex) when (True(() => logger.LogError(ex, "Failed to increment {inception_messageType} message counter.", messageType.Name))) { }
        }
        ...
        public async Task DecrementAsync(Type messageType, long decrementWith = 1)
        {
            ...
            catch (Exception ex) when (True(() => logger.LogError(ex, "Failed to decrement {inception_messageType} message counter.", messageType.Name))) { }
        }
        ...
        public async Task<long> GetCountAsync(Type messageType)
        {
            ...
            catch (Exception ex) when (True(() => logger.LogError(ex, "Failed to get {inception_messageType} message counter.", messageType.Name)))
            {
                return 0;
            }
        }
