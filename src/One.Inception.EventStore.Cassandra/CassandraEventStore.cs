...
namespace One.Inception.EventStore.Cassandra
{
    ...
    {
        public CassandraEventStore(IInceptionContextAccessor contextAccessor, TSettings settings, IndexByEventTypeStore indexByEventTypeStore, ILogger<CassandraEventStore> logger)
            : base(contextAccessor, settings.CassandraProvider, settings.TableNameStrategy, settings.Serializer, indexByEventTypeStore, logger)
        {
        }
    }
    ...
    public class CassandraEventStore : IEventStore, IEventStorePlayer
    {
        ...
        public CassandraEventStore(IInceptionContextAccessor contextAccessor, ICassandraProvider cassandraProvider, ITableNamingStrategy tableNameStrategy, ISerializer serializer, IndexByEventTypeStore indexByEventTypeStore, ILogger<CassandraEventStore> logger)
        {
            ...
            _loadAggregateEventsQuery = new LoadAggregateEventsQuery(contextAccessor, cassandraProvider, tableNameStrategy);
            _insertEventsQuery = new InsertEventsQuery(contextAccessor, cassandraProvider, tableNameStrategy);
            _loadEventsQuery = new LoadEventsQuery(contextAccessor, cassandraProvider, tableNameStrategy);
            _loadAggregateEventsWithinSpecifiedRevisionsQuery = new LoadAggregateEventsWithinSpecifiedRevisionsQuery(contextAccessor, cassandraProvider, tableNameStrategy);
            _loadAggregateRebuildEventsQuery = new LoadAggregateEventsRebuildQuery(contextAccessor, cassandraProvider, tableNameStrategy);
            _loadEventQuery = new LoadEventQuery(contextAccessor, cassandraProvider, tableNameStrategy);
            _deleteEventQuery = new DeleteEventQuery(contextAccessor, cassandraProvider, tableNameStrategy);
        }
        ...
        public async Task<AggregateEventRaw> LoadAggregateEventRaw(IndexRecord indexRecord)
        {
            ...
            logger.LogError("Unable to load aggregate event by index record: {1nception_messageData}", indexRecord.ToJson());
        ...
        private async Task<AggregateEventRaw> LoadAggregateEventRawAsync(IndexRecord indexRecord, PreparedStatement queryStatement, ISession session)
        {
            try
            {
                ...
                logger.LogError("Unable to load aggregate event by index record: {inception_messageData}", indexRecord.ToJson());
            }
            catch (Exception ex) when (True(() => logger.LogError(ex, "Unable to load aggregate event by index record: {inception_messageData}", indexRecord.ToJson()))) { }
            ...
            await foreach (IndexRecord indexRecord in indexByEventTypeStore.GetRecordsAsync(replayOptions, @operator.NotifyProgressAsync, cancellationToken))
            {
                if (@operator.OnLoadAsync is not null)
                {
                    ...
                    if (tasks.Count >= replayOptions.MaxDegreeOfParallelism)
                    {
                        ...
                        {
                            logger.LogError(completedTask.Exception, "Failed to replay event for index record: {inception_messageData}", indexRecord.ToJson());
                        }
                ...
                if (@operator.OnLoadAsync is not null)
                {
                    ...
                    if (tasks.Count >= replayOptions.MaxDegreeOfParallelism)
                    {
                        ...
                        if (completedTask.Status == TaskStatus.Faulted)
                        {
                            ...
                            logger.LogError(completedTask.Exception, "Failed to replay event: {inception_messageData}", dataAsJson);
                        }
                        tasks.Remove(completedTask);
                    }
                }
        ...
        class LoadEventQuery : PreparedStatementCache
        {
            ...
            public LoadEventQuery(IInceptionContextAccessor context, ICassandraProvider cassandraProvider, ITableNamingStrategy tableNameStrategy) : base(context, cassandraProvider, tableNameStrategy) { }
        ...
        class LoadAggregateEventsQuery : PreparedStatementCache
        {
            ...
            public LoadAggregateEventsQuery(IInceptionContextAccessor context, ICassandraProvider cassandraProvider, ITableNamingStrategy tableNameStrategy) : base(context, cassandraProvider, tableNameStrategy) { }
        ...
        class InsertEventsQuery : PreparedStatementCache
        {
            ...
            public InsertEventsQuery(IInceptionContextAccessor context, ICassandraProvider cassandraProvider, ITableNamingStrategy tableNameStrategy) : base(context, cassandraProvider, tableNameStrategy) { }
        ...
        class LoadEventsQuery : PreparedStatementCache
        {
            ...
            public LoadEventsQuery(IInceptionContextAccessor context, ICassandraProvider cassandraProvider, ITableNamingStrategy tableNameStrategy) : base(context, cassandraProvider, tableNameStrategy) { }
        ...
        class LoadAggregateEventsWithinSpecifiedRevisionsQuery : PreparedStatementCache
        {
            ...
            public LoadAggregateEventsWithinSpecifiedRevisionsQuery(IInceptionContextAccessor context, ICassandraProvider cassandraProvider, ITableNamingStrategy tableNameStrategy) : base(context, cassandraProvider, tableNameStrategy) { }
        ...
        class LoadAggregateEventsRebuildQuery : PreparedStatementCache
        {
            ...
            public LoadAggregateEventsRebuildQuery(IInceptionContextAccessor context, ICassandraProvider cassandraProvider, ITableNamingStrategy tableNameStrategy) : base(context, cassandraProvider, tableNameStrategy) { }
        ...
        class DeleteEventQuery : PreparedStatementCache
        {
            ...
            public DeleteEventQuery(IInceptionContextAccessor context, ICassandraProvider cassandraProvider, ITableNamingStrategy tableNameStrategy) : base(context, cassandraProvider, tableNameStrategy) { }
