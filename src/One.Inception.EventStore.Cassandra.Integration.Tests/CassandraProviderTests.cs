...
public class CassandraProviderTests
{
    ...
    public async Task GetSessionWithNoKeyspaceNamingAsync()
    {
        ...
        {
            Context = new InceptionContext(tenant, new NullServiceProviderMock())
        };
        var keyspaceNaming = new NoKeyspaceNamingStrategy();
...
class ContextAccessorMock : IInceptionContextAccessor
{
    public InceptionContext Context { get; set; }
}
